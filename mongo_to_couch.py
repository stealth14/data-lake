...
SKIPPED = []
argp = ArgumentParser()
argp.add_argument("-c", "--couchdb", help="couchdb endpoint, example: http://admin:ander123@localhost:5984", required=True)
...
DBS = ['music','politico','politica','spotify','ecuador']  # for pymongo version > 3.4.0, replace this line with DBS = CLIENT.list_database_names()
    ...
    if db not in ('admin', 'local'):  # omitting  special internal db's, 'admin'(DBS[0]) and 'local'(DBS[2])
        ...
        for col in cols:
            ...
            for x in CLIENT[db][col].find():  # iterate though all records inside a collection
                ...
                try:
                    response = requests.Session().get(URL+'/'+db+'-'+col, auth=('admin', 'ander123'), headers=HEADERS, verify=False)
                    if 'error' in response.json():  # db doesnt exist on destination, create it
                        print('Creating a new database {} in CouchDB'.format(db+'-'+col))
                        output = requests.Session().put(URL+'/'+db+'-'+col, auth=('admin', 'ander123'), headers=HEADERS, verify=False)
                        if output.status_code == 201 or 202:
                            DB_COUNT += 1
                    # insert documents in couchDB database.
                    output = requests.Session().post(URL+'/'+db+'-'+col, auth=('admin', 'ander123'), headers=HEADERS, data=documents, verify=False)
                    if output.status_code == 201 or 202:
                        DOCUMENT_COUNT += 1
